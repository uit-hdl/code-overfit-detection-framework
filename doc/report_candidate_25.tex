...
But, how robust are the mentioned explanations?~\cite{geometryToBlame} finds that it is possible to modify input images in a way that is (barely) unnoticable to humans but nevertheless dramatically modifies the output explanations. One such perturbation is shown in~\Cref{fig:arbitrary}. There has therefore been made several metrics for getting quantifiable verifications of how good an explanation is. Several of these are implemented in Quantus~\cite{hedstrom2023quantus}, a python API that lets you automatically get metrics for 6 different dimensions of explanations:
\begin{enumerate}
...
% Yet another solution is \gls{SSL}, which has distorted inputs together with the original data, e.g. in image processing this could be by rotating or cropping the image, or occluding parts of the image. It should still be possible (at least for humans) to understand what the core feature/object of the image is, otherwise the model wouldn't learn any features. 
...
% TODO: I attack them for their tile choice.
...
%Setting batch size = 1 occupied around 27 GB of main memory on my computer, which only has around 32 GB total memory. When writing the model to disk, it occupied a few additional GB - grinding my computer down to a halt. I tried another machine with 128 GB available,
...
For evaluating \gls{UMAP} plots, I tried every metric listed in SciPy's ``spatial distance module'' ~\cite{scipy} and several other inspired from section $2.9$ in~\cite{datarepresentativity} (which was partially covered in~\Cref{sec:representative}). The ones I ended up finding the most \textit{indicative} when evaluating different samples were:
\begin{enumerate}
...
\Cref{fig:defaultLoss} shows the loss output from training a model using similar parameters as~\cite{sslUMAP}. It seems to stabilize after the 50 last epochs, although it is possible that an extra few hundred epochs would reduce the loss down even further. This was not done in an effort to keep models as close to the original paper as possible.

For this project, I use \textit{random\_state = 42} in the python \gls{UMAP} API, version 0.5.

\subsection{Results}\label{sec:results}

For comparative evaluation, five different models have been trained. Model $\alpha$ is the same as the optimal model from~\cite{sslUMAP}, except that the batch size is set to 1 rather than 4 as discussed in~\Cref{subsec:mymodel}. $\beta$ is a model trained on only 3 photos (yet still only 1 image per batch), $\gamma$ is a model similar to $\alpha$ except it has only used crop and color distortions as image augmentations (similar as in the recommended section from~\cite{simCLR}), $\delta$ is a model trained using unconditional \gls{SSL}, i.e. random sampling of slides and $\epsilon$ is a model similar to $\alpha$, except it's only trained for one epoch. $\epsilon$ is only used to a biased, clustered photos for~\Cref{fig:umaps}.

% \gls{UMAP} plots are generated by feeding in all points from slides that are going to be used in the \gls{UMAP} figure. Another option would be to insert points from \textit{all} slides into a photo and then visually representing a subset of these. This wasn't done as I didn't want because of .% TODO SECTION UMAP
...
\begin{figure*}
  ...
  \begin{subfigure}{.25\textwidth}
    \centering
    \umapSubPlot{fig:biasedUmap}{heavilyBiasedUmap}{4}{$\epsilon$ (only run on three slides)}
  \end{subfigure}
  \hfill
  \begin{subfigure}{.25\textwidth}
    \centering
...
\centering
  \begin{tabular}{|l l l l l l|}
    ...
    \hline
  \end{tabular}
  \caption{Quantitative measures/means from the \gls{UMAP} figures in~\Cref{fig:umaps}. \textit{Shannon} would not be computed for some sets, thus left blank}
  \label{tab:quantitativeres}
\end{table*}

It can be difficult to tell the different models apart from the figures in~\Cref{fig:umaps}. $\epsilon$ is the only one to indubitably stand out with clear clustering from each slide. The numbers from~\Cref{tab:quantitativeres} capture this in the Hausdorff, Frechet and Wasserstein distance that are all significantly higher for $\epsilon$ than for any other trained model. The cosine similarity stands out in model $\gamma$. This \textit{may} be used as an interpretation that the dataset is less biased, however, the results section in~\Cref{tab:combinedres} are still low for $\gamma$ compared to others. 

% TODO: put in slide 6 scores?
% We can see (arguably) that slide 6 is clustered to an extent in all models. 
...
From~\Cref{tab:combinedres} we see that $\beta$ is the model with the highest C-index. This is highly unexpected as it only has had three images for it's input during training. It's brier-score, is worse than for all others. This could indicate that a C-index is not sufficient alone for model evaluation, unless the three slides for training were somehow entirely representative.
