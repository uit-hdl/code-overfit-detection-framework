...
\begin{comment}
In recent years, the interest in eXplainable AI (XAI) techniques has undoubtedly exploded and countless methods and toolboxes are now at the disposal of XAI researchers, ML practitioners and data scientists alike. Despite my shared enthusiasm for this productive development, until recently, the topic of XAI evaluation was grossly understudied — and caused confusion about which explanation methods work and under what conditions. In this tutorial, we will take an in-depth look at some of the most recent developments in XAI evaluation and review the solutions that the community has put forward to circumvent the fundamental problem of XAI — the lack of ground-truth explanations. In addition to this, we will discuss The Challenge of Unverfiability — why evaluation is so difficult to get right (and what we can do about it). At the end of the tutorial, a brief yet hands-on introduction to Quantus will be given, which is an open-source library intended for XAI researchers to evaluate local neural network explanations.
\end{comment}
...
% this is also contrastive learning
\gls{SSL} is covered in~\Cref{sec:ssl}. One way to make \gls{SSL} work is to use distorted inputs together with the original data, e.g. in image processing this could be by rotating or cropping the image, or occluding parts of the image. It should still be possible (at least for humans) to understand what the core feature/object of the image is, otherwise the model wouldn't learn any features. SimCLR(CITE) has a nice table to show which alterations/augmentations work better or worse than others for ImageNet. The alterations are also called \textit{human priors}, since they rely on keeping features that humans need to understand the object. While this has gotten good results on natural images, there are some domains such as medicine where the augmentations are less efficient. Especially in histopathology where there are many small details in each section of an image that are all important to make a classification.


% Representation learning is a type of machine learning technique that aims to automatically learn meaningful representations or features of raw data. In traditional machine learning, humans usually engineer or handcraft features that are used as input to the learning algorithm. However, with representation learning, the algorithm itself learns these features from the raw data.
% 
% The goal of representation learning is to transform the raw input data into a more meaningful and useful representation, which captures the underlying structure and patterns of the data. The learned representation should be informative, efficient, and generalizable to new and unseen data.
% 
% There are different types of representation learning techniques, including:
% 
%     Autoencoders: These are neural networks that learn to reconstruct their input data from a compressed representation or bottleneck. The bottleneck is typically a low-dimensional vector that captures the most important features of the input data.
% 
%     Convolutional Neural Networks (CNNs): These are neural networks that are particularly well-suited for learning representations from image data. They use convolutional layers that learn to extract local features from the image, which are then combined and transformed in subsequent layers to produce a high-level representation.
% 
%     Recurrent Neural Networks (RNNs): These are neural networks that are used for learning representations from sequential data such as text or time series. They use recurrent connections to maintain a memory of past inputs, and learn to extract meaningful representations from the sequence.
% 
% Representation learning has become an important technique in many areas of machine learning, including computer vision, natural language processing, and speech recognition. By automatically learning informative representations from raw data, representation learning can help improve the accuracy and efficiency of machine learning algorithms, and enable them to tackle more complex and challenging tasks.
