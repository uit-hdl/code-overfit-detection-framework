...
do
  ...
  do
    ...
    cd ~/feature-inspect
    ipython ~/feature-inspect/examples/use_case_linear_probe.py -- --embeddings-path $embed_dir/inception_tiles_${sz}_${run}_embedding.zarr/ --label-file $labelfile --label-key institution --out-dir lp_final/out_${sz}_inception_$run --epochs 60 --batch-size 256 --tensorboard-name "${sz}_${run}_inception"
    ipython ~/feature-inspect/examples/use_case_linear_probe.py -- --embeddings-path $embed_dir/phikon_tiles_${sz}_${run}_embedding.zarr --label-file $labelfile --label-key institution --out-dir lp_final/out_${sz}_phikon_$run --epochs 60 --batch-size 256 --tensorboard-name "${sz}_${run}_phikon"
