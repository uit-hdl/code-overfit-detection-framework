...
\foreach \x in {1,...,9}
{
  ...
  \fi
    \fi
  \node[yslant=0.5, fill=\rectColor, opacity=0.8] (wsi\x) at (\x-4,0) 
  {\includegraphics[width=.1\textwidth, height=.1\textheight]{./wsi\x.png}};
}


\draw[decorate, decoration={brace, amplitude=10pt, mirror}]
    (-4,-1.5) -- (5.3,-1.5) node [midway, below=10pt] (Train) {Training data} ;
    ...
    \begin{scope}[yshift=\y*\shiftY]
        ...
        \else
          \ifnum\y<3
            ...
            \node[anchor=north] at ([shift={(-0.8,0.5)}]cswsi\y1.west) {Batch \number\numexpr\y\relax};
        \else
            \node[anchor=north] at ([shift={(-0.8,-1.5)}]cswsi21.west) {Batch \number\numexpr\y\relax};
            % ([shift={(-0.2,\y*\shiftY+0.5)}]rBase) 
              \fill ([shift={(1,\y*\shiftY)}]rBase)  circle (2pt);
              \fill ([shift={(.2*\shiftX+1.5,\y*\shiftY)}]rBase)  circle (2pt);
              \fill ([shift={(.3*\shiftX+2.0,\y*\shiftY)}]rBase)  circle (2pt);
          \fi
          \fi
    \end{scope}
}
