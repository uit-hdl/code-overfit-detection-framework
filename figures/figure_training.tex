    ...
    \foreach \y in {1,...,3}
    {
        \begin{scope}[yshift=\y*\shiftY]
            \foreach \x in {1,...,2}
            {
              ...
              \ifnum\y<2
                \node[yslant=0.5, fill=orange] (cswsi\y3) at ([shift={(2*\shiftX+0.5,-6+\y*\shiftY)}]csBase) 
                {
                  {\includegraphics[width=.05\textwidth, height=.05\textheight]{./wsi5.png}}
            };
                \node[yslant=0.5, fill=blue] (cswsi\y4) at ([shift={(2*\shiftX+1,-6+\y*\shiftY)}]csBase) 
                {
                  {\includegraphics[width=.05\textwidth, height=.05\textheight]{./wsi3.png}}
            };
              \else
                \ifnum\y<3
                \node[yslant=0.5, fill=green] (cswsi\y3) at ([shift={(2*\shiftX+0.5,-6+\y*\shiftY)}]csBase) 
                {
                  {\includegraphics[width=.05\textwidth, height=.05\textheight]{./wsi2.png}}
            };
                \node[yslant=0.5, fill=orange] (cswsi\y4) at ([shift={(2*\shiftX+1,-6+\y*\shiftY)}]csBase) 
                {
                  {\includegraphics[width=.05\textwidth, height=.05\textheight]{./wsi5.png}}
            };
              \else
...
\node [draw, diamond, aspect=2] {Use conditional sampling?};
