...
The authors offer a pretrained network. This network is an InceptionV4 model that can be used an encoder in \gls{MOCO}. However, they do not specify explicitly what the model is trained on - assumably it would be the data used in their paper.

\subsubsection{Code}\label{subsubsec:code}
Much of the code did not work out the box. The kind of errors I ran into were missing imports, file and directory names that were mangled (trying to access files in a directory and subsequently looking for the same files in a different directory) and other errors such as API mismatch when trying to load \textit{pickled}\footnote{serialized Python objects, written to file for later use, see \href{https://docs.python.org/3/library/pickle.html}{https://docs.python.org/3/library/pickle.html}} files directly into Pytorch. The pretrained weights they provided was also not compatible? Missing imports might stem from having written code in an environment with hidden files, the file mangling might be because they had duplicate files scattered in their directories and the API errors might arise from using different versions of third-party tools. Another possibility is that the code that is published is not tested or used after it has been modified at some point. This seems to make sense as there are also some references to some data files that aren't created, such as ``2 year splits'' of images.
...
\subsubsection{Misconceptions}\label{subsubsec:misconception}
The training script listed in the code repository, ``\textit{train\_ssl.py}'' (also referenced in the README documentation) trains a \gls{MOCO} model. The inference script ``\textit{extract\_embeddings.py}'', however, only uses the encoding model from the training, an \textit{InceptionV4} model.  It seems deliberate as there is specific code written to extract the InceptionV4 from a pickled \gls{MOCO} model.

Two models are used for feature extraction in their scripts. An InceptionV4 model with two output neurons is used to identify cancer vs non-cancer in a tile. Then a second InceptionV4 with 1536 output neurons (the number 1536 comes from the SimCLR model in~\cite{simCLR})
