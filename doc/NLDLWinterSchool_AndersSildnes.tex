...
The first thing I did when re-creating the work was to get the code from their listed website \href{https://github.com/NYUMedML/conditional_ssl_hist}. I then ran into the following difficulties:
...
The training script listed in the code repository, ``\textit{train_ssl.py}'' (also referenced in the README documentation) trains a \gls{MOCO} model. The inference script ``\textit{extract_embeddings.py}'', however, only uses the encoding model from the training, an \textit{InceptionV4} model.  It seems deliberate as there is specific code written to extract the InceptionV4 from a pickled \gls{MOCO} model.

Two models are used for feature extraction in their scripts. An InceptionV4 model with two output neurons is used to identify cancer vs non-cancer in a tile. Then a second InceptionV4 with 1536 output neurons? Is used to.
