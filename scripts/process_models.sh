...
for model in model_out2b1413ba2b3df0bcd9e2c56bdbea8d2c7f875d1e/MoCo/tiles//model/checkpoint_MoCo_tiles_0100_True_m256_n0_o4_K256.pth.tar out-no-color-normalization/MoCo/tiles-no-color-normalization/model/checkpoint_MoCo_tiles-no-color-normalization_0040_False_m64_n0_o0_K64.pth.tar
do
  ...
  test -d "${out_dir}"/MoCo/*/model/relabelled_my_inst*${bsrc}*/ || \
  ...
  then
	  ...
	  test -d "${out_dir}"/MoCo/*/model/relabelled_my_inst*${balt}*/ || \
		  ipython train_model/relabelling.py -- --epochs 5 --src-dir "${SRC_DIR}" --out-dir "${out_dir}" --feature_extractor "${model}" --label-key "my_inst"
